...
export class UserGenrePreferencesService {
  ...
  async listForUser(userId: string) {
    return this.prefRepository.find({ where: { user: { id: userId } }, relations: { genre: true } });
  }

  async upsert(user: User, genreId: number, preferenceLevel: number) {
    ...
    let pref = await this.prefRepository.findOne({ where: { user: { id: user.id }, genre: { id: genreId } }, relations: { user: true, genre: true } });
    if (!pref) {
      pref = this.prefRepository.create({ user, genre, preferenceLevel });
    } else {
      pref.preferenceLevel = preferenceLevel;
    }
    return this.prefRepository.save(pref);
  }

  async remove(userId: string, genreId: number) {
    const pref = await this.prefRepository.findOne({ where: { user: { id: userId }, genre: { id: genreId } } });
    if (!pref) {
      return;
    }
